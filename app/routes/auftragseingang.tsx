...
export async function action({ request }: { request: Request }) {
  ...
  if (intent === "repairImportedOrders") {
    const { extractUniversalOrder } =
      await import("../lib/order-import-extract.server");
    ...
    const isPlaceholderCustomer = (value: unknown) => {
      const normalized =
        normalizeCustomerValue(value);

      return (
        !normalized ||
        normalized === "kunde prufen" ||
        normalized === "kunde pruefen" ||
        normalized === "kunde unbekannt" ||
        normalized === "unzugeordneter e mail auftrag" ||
        normalized === "e mail import" ||
        normalized === "email import"
      );
    };

    const isCompanyLikeCustomer = (value: unknown) => {
      const customerName =
        String(value || "")
          .replace(/\s+/g, " ")
          .trim();
      ...
      if (
        ...
        isPlaceholderCustomer(customerName)
      ) {
        return false;
      }

      const normalized =
        normalizeCustomerValue(customerName);

      if (
        normalized.includes("edis gastrobetriebe") ||
        normalized.includes("heycater") ||
        normalized.includes("hey group") ||
        normalized.includes("gastario")
      ) {
        return false;
      }

      return /\b(gmbh|ug|ag|se|kg|ohg|gbr|ev|inc|ltd|llc|group|holding|management|office|solutions|studio|company)\b/i.test(
        customerName
      );
    };
    ...
    for (const order of repairOrders) {
      ...
      const attachmentTexts =
        Array.isArray(incomingEmail.attachments)
      ...
      const extractedOrder =
        extractUniversalOrder(sourceText);
        ...
        customers.find((customer) => {
          ...
          return (
            normalizedCustomerName ===
              normalizedExtractedName ||
            (
              normalizedCustomerName.length >= 5 &&
              normalizedExtractedName.includes(
                normalizedCustomerName
              )
            ) ||
            (
              normalizedExtractedName.length >= 5 &&
      ...
      const extractedTotalCents =
        Math.max(
          0,
          Number(
            extractedOrder?.pdfNetTotalCents ||
            itemTotalCents ||
            0
          )
        );
      ...
      if (
        extractedTotalCents > 0 &&
        extractedTotalCents !==
          Number(order.totalCents || 0)
      ) {
        updateData.totalCents =
          extractedTotalCents;

        totalRepairedCount += 1;
      }
    ...
    return {
      success:
        repairedCount +
        " bestehende Import-Aufträge wurden neu ausgewertet. " +
        customerMatchedCount +
        " Kunden wurden mit dem Kundenstamm verbunden. " +
        totalRepairedCount +
        " Auftragssummen wurden korrigiert. " +
        skippedCount +
        " Vorgänge blieben unverändert.",
    };
  }
                ...
                <button
                  ...
                  onClick={(event) => {
                    const confirmed = window.confirm(
                      "Bestehende fehlerhafte Import-Aufträge erneut auswerten und erkannte Kunden sowie Summen korrigieren?"
                    );
