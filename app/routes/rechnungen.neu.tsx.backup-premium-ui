...
export async function action({ request }: { request: Request }) {
  ...
  if (invoiceItems.filter((item) => item.type === "ITEM").length === 0) {
    return { error: "Mindestens eine Artikelposition fehlt." };
  }
  ...
  return { success: "Rechnungsentwurf wurde gespeichert. Kundennummer: " + customerNumber };
}
    ...
    <AppLayout>
      <header className="topbar">
        <div>
          ...
          <p className="muted">Lexoffice-ähnlicher Editor mit Live-Berechnung.</p>
        </div>
      ...
      <Form method="post" style={pageStyle}>
        ...
        <section style={positionCardStyle}>
              ...
              row.type === "text" ? (
                <div key={row.id} style={textRowStyle}>
                  ...
                  <FloatingControlledInput
                    name="itemName"
                    label="Freitext"
                    placeholder="Text zur Rechnung"
                    value={row.text}
                    onChange={(value) => updateText(row.id, value)}
                  />
          ...
          <div style={positionActionsStyle}>
            <button type="button" onClick={addItem} style={greenOutlineButtonStyle}>+ ARTIKEL</button>
...
const pageStyle: React.CSSProperties = { maxWidth: 1180, margin: "0 auto", display: "grid", gap: 16 };
const topSwitchStyle: React.CSSProperties = { display: "flex", justifyContent: "flex-end", gap: 0 };
const switchButtonStyle: React.CSSProperties = { border: "1px solid #cbd5e1", padding: "10px 18px", background: "#ffffff", fontWeight: 800, cursor: "pointer" };
const activeSwitchButtonStyle: React.CSSProperties = { ...switchButtonStyle, background: "#e6f4ef", color: "#047857", boxShadow: "inset 0 0 0 1px #059669" };
const cardStyle: React.CSSProperties = { background: "#ffffff", border: "1px solid #d7dde5", borderRadius: 8, padding: 18, boxShadow: "0 1px 2px rgba(15,23,42,0.05)" };
const positionCardStyle: React.CSSProperties = { ...cardStyle, padding: 0, overflow: "hidden" };
const twoColStyle: React.CSSProperties = { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 78 };
...
const floatingLabelStyle: React.CSSProperties = { display: "grid", gap: 4, color: "#64748b", fontSize: 12, fontWeight: 750 };
const inputStyle: React.CSSProperties = { width: "100%", height: 50, border: "1px solid #cbd5e1", borderRadius: 4, padding: "0 13px", fontSize: 16, fontWeight: 500, background: "#ffffff" };
...
const numberCircleStyle: React.CSSProperties = { width: 32, height: 32, borderRadius: 999, background: "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900, marginBottom: 9 };
...
const taxPillStyle: React.CSSProperties = { border: "none", borderRadius: 999, background: "#3f3f3f", color: "#ffffff", height: 28, padding: "0 9px", fontWeight: 800 };
const deleteButtonStyle: React.CSSProperties = { border: "none", background: "transparent", color: "#64748b", fontSize: 20, fontWeight: 900, cursor: "pointer", marginBottom: 12 };
const positionActionsStyle: React.CSSProperties = { display: "flex", justifyContent: "center", gap: 26, padding: "10px 22px 26px" };
const greenOutlineButtonStyle: React.CSSProperties = { border: "1px solid #10a66a", background: "#ffffff", color: "#059669", borderRadius: 4, padding: "8px 18px", fontWeight: 900, cursor: "pointer" };
const textActionStyle: React.CSSProperties = { border: "none", background: "transparent", color: "#334155", fontWeight: 900, cursor: "pointer" };
...
const paymentGridStyle: React.CSSProperties = { display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 16 };
const paymentCardStyle: React.CSSProperties = { background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: 14, padding: 20, display: "flex", justifyContent: "space-between" };
const footerActionsStyle: React.CSSProperties = { display: "flex", justifyContent: "flex-end", gap: 12, padding: "0 0 40px" };
const primaryButtonStyle: React.CSSProperties = { border: "none", background: "#059669", color: "#ffffff", borderRadius: 5, padding: "12px 18px", fontWeight: 950, cursor: "pointer" };
const secondaryButtonStyle: React.CSSProperties = { border: "1px solid #cbd5e1", background: "#ffffff", color: "#334155", borderRadius: 5, padding: "12px 18px", fontWeight: 850, textDecoration: "none" };
