...
export async function action({ request }: { request: Request }) {
  ...
  await prisma.$executeRawUnsafe(
    `INSERT INTO "FoodProductLabel"
      ("id", "tenantId", "name", "ingredients", "allergens", "logoDataUrl", "labelCount", "printPreset", "publicToken", "updatedAt")
     VALUES
      ...
      <section style={pageGridStyle}>
        ...
        <div style={editorCardStyle}>
          ...
          <Form method="post" encType="multipart/form-data" style={formGridStyle}>
            ...
            <Field label="Zutaten">
              <input name="ingredients" placeholder={"z. B. Reis, H\\u00e4hnchen, Gem\\u00fcse, Sauce"} />
            </Field>
                  ...
                  <div key={label.id} style={labelRowStyle}>
                    ...
                    <div style={listActionGroupStyle}>
                      ...
                      <Form
                        method="post"
                        onSubmit={(event) => {
                          if (!window.confirm("Dieses Foodlabel wirklich loeschen?")) {
            ...
            {selected ? (
              <>
                <div style={previewWrapStyle}>
                  <FoodLabelCard label={selected} tenantName={data.tenantName} />
                </div>
...
function FoodLabelCard({ label, tenantName }: { label: any; tenantName: string }) {
  ...
  return (
    <article style={{ ...foodLabelCardStyle, width: preset.width, height: preset.height, fontSize: preset.base }}>
      <div style={foodTopStyle}>
        {label.logoDataUrl ? <img src={label.logoDataUrl} alt="" style={{ ...logoStyle, width: preset.logo, height: preset.logo }} /> : <div style={{ ...logoPlaceholderStyle, width: preset.logo, height: preset.logo }}>Logo</div>}
        <div style={foodHeaderTextStyle}>
          <strong style={{ ...foodNameStyle, fontSize: preset.title }}>{label.name}</strong>
