...
// 2) Server: neue Felder beim Auftrag speichern
...
// 3) UI-Block direkt vor POSITIONEN einfügen
const billingBlock = `
            <div style={{
...
if (!text.includes("Rechnung & Steuer")) {
  const positionMarker = `            <div style={{ display: "grid", gap: 10 }}>
              <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>`;

  const index = text.indexOf(positionMarker);

  if (index === -1) {
    throw new Error("Positionen-Bereich nicht gefunden.");
  }

  text = text.slice(0, index) + billingBlock + "\n" + text.slice(index);
}
