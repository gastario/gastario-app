...
const newParser = `function isAddressLine(value: string) {
...
function parseOneHeycaterLabel(date: string, block: string[]) {
  ...
  // Fallback fuer seltene PDF-Reihenfolge:
  // Wenn Gericht fehlt, nimm die erste sinnvolle Zeile nach Adresse, die keine Info/Meta ist.
  if (!meal && address) {
...
fs.writeFileSync(path, before + newParser, "utf8");
console.log("Heycater Parser hart nach Datum -> Name -> Adresse -> Gericht -> Info gebaut.");
