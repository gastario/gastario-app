...
// Alten Filterblock robust ersetzen
const start = content.indexOf('          <div className="filterActions">');
const tableStart = content.indexOf('        <div className="ordersTable">', start);
...
if (tableStart === -1) {
  throw new Error("ordersTable Start nicht gefunden.");
}

const newFilterArea = `          <div className="ordersFilterWrap">
...
content = content.slice(0, start) + newFilterArea + content.slice(tableStart);

// CSS ergänzen
if (!content.includes("/* orders-filter-final */")) {
  const css = `
      <style>{\`
        /* orders-filter-final */
        .panelHeader {
...
console.log("Aufträge: Filterleiste final ersetzt.");
